...
if ("Notification" in window && Notification.permission === "default") {
  ...
  async init() {
    ...
    if (window.TickerPanel) {
        ...
        await this.tickerPanel.init();
        console.log('✅ TickerPanel инициализирован');
    } else {
        console.error('❌ window.TickerPanel не найден! Проверьте подключение скрипта');
        this.tickerPanel = {
    ...
    setTimeout(() => {
        if (this.chartManager && this.chartManager.chart) {
            ...
            this.chartManager.chart.applyOptions({ width, height });
            this.chartManager.chart.timeScale().fitContent();
            this.chartManager.scrollToLast();
            console.log('✅ Размеры графика исправлены:', width, height);
        }
    }, 500);
...
async loadInitialData() {
    ...
    if (data && data.length > 0) {
        ...
        if (this.wsManager) {
            this.wsManager.updateSymbol(defaultSymbol);
            this.wsManager.updateTimeframe(defaultInterval);
        }
    ...
    }  // ← ЗДЕСЬ ЗАКРЫВАЕТСЯ if (data && data.length > 0)
}      // ← ЗДЕСЬ ЗАКРЫВАЕТСЯ МЕТОД loadInitialData()
...
async initDrawingTools() {  // ← ТОЛЬКО ДОБАВИТЬ async
    ...
    setupToolButtons() {
        ...
        if (magnetBtn) {
            ...
            if (window.textManager) window.textManager.setMagnetEnabled(true);
        }
...
async loadSymbol(symbol, exchange, marketType, externalSignal = null) {
    ...
    try {
        ...
        if (this.wsManager) {
            this.wsManager.updateSymbol(symbol);
        }
