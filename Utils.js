...
};  // ← ЗАКРЫВАЕМ Utils ЗДЕСЬ

// ========== ГЛОБАЛЬНЫЕ ФУНКЦИИ (ВНЕ Utils) ==========
...
// ФУНКЦИЯ ПОЛУЧЕНИЯ PRECISION С БИРЖИ
async function getPrecisionFromExchange(symbol, exchange, marketType) {
    const cacheKey = `${exchange}_${marketType}_${symbol}`;
    
    if (PRECISION_CACHE.has(cacheKey)) {
        return PRECISION_CACHE.get(cacheKey);
    }
    ...
    try {
        ...
        if (exchange === 'binance') {
            ...
            if (priceFilter?.tickSize) {
                ...
                precision = Math.round(Math.log10(1 / tickSize));
            }
        }
        
        PRECISION_CACHE.set(cacheKey, precision);
        return precision;
    } catch (error) {
        return 2;
    }
