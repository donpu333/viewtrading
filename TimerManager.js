...
class TimerRenderer {
    ...
    draw(target) {
        ...
        target.useBitmapCoordinateSpace(scope => {
            ...
            // ========== ИСПРАВЛЕНИЕ: ПОЛУЧАЕМ Y-КООРДИНАТУ НАПРЯМУЮ ИЗ CHARTMANAGER ==========
            let rectY;
            
            // Если есть сохранённая Y-координата линии цены - используем её
            if (chartManager._lastPriceYCoordinate !== undefined && chartManager._lastPriceYCoordinate !== null) {
                const yCoordinate = chartManager._lastPriceYCoordinate;
...
class TimerPrimitive {
    ...
    _startPriceTracking() {
        const checkPosition = () => {
            if (!this._chartManager) return;
            
            // Получаем текущую Y-координату цены
            const price = this._chartManager.currentRealPrice;
            if (!price) {
            ...
            const currentYCoord = activeSeries.priceToCoordinate(price);
            
            // Сохраняем Y-координату в chartManager для синхронизации
            this._chartManager._lastPriceYCoordinate = currentYCoord;
            
            // Перерисовываем только если координата реально изменилась
            if (currentYCoord !== this._lastYCoord) {
                this._lastYCoord = currentYCoord;
                this.requestRedraw();
            }
