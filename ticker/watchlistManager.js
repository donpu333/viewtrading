class WatchlistManager {
    ...
    async activateList(listId) {
        ...
        // Сохраняем текущие символы в старый список перед уходом
        ...
        // ✅ ИСПРАВЛЕНИЕ: Обновляем массив отображаемых символов
        this.tickerPanel.state.customSymbols = [...newList.symbols];

        // ✅ ИСПРАВЛЕНИЕ: Очищаем ТОЛЬКО массив рендера и DOM, НО НЕ ПАМЯТЬ (tickersMap)!
        ...
        // Наполняем массив рендера из глобальной памяти (tickersMap)
        newList.symbols.forEach(symbolKey => {
            ...
            if (this.tickerPanel.tickersMap.has(symbolKey)) {
                // ✅ Монета УЖЕ в памяти (из другого списка). Берем её с живой ценой!
                this.tickerPanel.tickers.push(this.tickerPanel.tickersMap.get(symbolKey));
            } else {
                // Монеты нет в памяти. Создаем с 0 и добавляем в память
                const newTicker = { 
        ...
        // Подтягиваем цены только для тех монет, которых только что добавили в память
    ...
    loadSymbolsFromList(listId) {
        ...
        // ✅ ИСПРАВЛЕНИЕ: Очищаем только массив рендера, НО НЕ ПАМЯТЬ (tickersMap)!
        this.tickerPanel.tickers = []; 
        ...
        list.symbols.forEach(symbolKey => {
            ...
            if (parts.length === 3) {
                ...
                // Если монета уже загружена в память — берем оттуда (с ценой!)
                if (this.tickerPanel.tickersMap.has(symbolKey)) {
                    this.tickerPanel.tickers.push(this.tickerPanel.tickersMap.get(symbolKey));
                } else {
                    // Иначе добавляем через addSymbol (создаст с 0 и запросит цену)
