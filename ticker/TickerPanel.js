...
this.watchlistManager = new WatchlistManager(this);
  ...
  async init() {
    ...
    // Рендерим вкладки вотчлистов сразу
if (this.watchlistManager) {
    ...
    setTimeout(async () => {
        ...
        // Приоритетная загрузка вотчлистов
        if (this.watchlistManager) {
...
addInitialSymbols() {
  ...
  clearAllSymbols() {
   ...
   addSymbol(symbol, isCustom = true, exchange = 'binance', marketType = 'futures', render = true, skipInitialFetch = false) {
    ...
    if (isCustom && this.watchlistManager) {
...
async addSymbolsBatch(symbolsData) {
  ...
  handleContextMenu(e) {
    ...
    // Если кликнули НЕ по флагу и НЕ по плейсхолдеру — блокируем меню
