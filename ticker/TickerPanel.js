class TickerPanel {
    constructor(coordinator) {
       ...
       async init() {
        ...
        this.setupModal();
            ...
            // ====================================================================
            // ИСПРАВЛЕНИЕ: Если кликнули прямо на текст (цифры/буквы), 
            // e.target - это текстовый узел, у него нет метода .closest().
            // Подменяем его на родительский тег (например, <div class="ticker-price">)
            // ====================================================================
            let target = e.target;
            ...
            if (tickerItem) {
                e.preventDefault(); // Теперь это сработает для текста!
   ...
   async initializeDataParallel() {
    ...
    startTickerPanelPriceEngine() {
        if (this._priceEngineInterval) clearInterval(this._priceEngineInterval);
        
        this._priceEngineInterval = setInterval(async () => {
            if (this.tickersMap.size === 0) return; // Если пусто - не грузим
            
            const bnFut = new Set(), bnSpot = new Set(), byFut = new Set(), bySpot = new Set();
            for (const [key] of this.tickersMap.entries()) {
                const [s, e, m] = key.split(':');
                if (e === 'binance' && m === 'futures') bnFut.add(s);
                else if (e === 'binance' && m === 'spot') bnSpot.add(s);
                else if (e === 'bybit' && m === 'futures') byFut.add(s);
                else if (e === 'bybit' && m === 'spot') bySpot.add(s);
            }
            
            const promises = [];
            
            // Binance Futures & Spot: Цены
            if (bnFut.size > 0 || bnSpot.size > 0) {
                promises.push(
                    fetch('https://fapi.binance.com/fapi/v1/ticker/price').then(r=>r.json()).then(data => {
                        if(!Array.isArray(data)) return;
                        data.forEach(t => {
                            if(bnFut.has(t.symbol)) { const tk=this.tickersMap.get(`${t.symbol}:binance:futures`); if(tk) tk.price=parseFloat(t.price); }
                            if(bnSpot.has(t.symbol)) { const tk=this.tickersMap.get(`${t.symbol}:binance:spot`); if(tk) tk.price=parseFloat(t.price); }
                        });
                    }).catch(()=>{})
                );
                // Binance Futures: Изменения за 24ч
                if(bnFut.size > 0) {
                    promises.push(fetch('https://fapi.binance.com/fapi/v1/ticker/24hr').then(r=>r.json()).then(data => {
                        if(!Array.isArray(data)) return;
                        data.forEach(t => { if(bnFut.has(t.symbol)) { const tk=this.tickersMap.get(`${t.symbol}:binance:futures`); if(tk){ tk.change=parseFloat(t.priceChangePercent); tk.volume=parseFloat(t.quoteVolume); tk.trades=parseInt(t.count); }}});
                    }).catch(()=>{}));
                }
                // Binance Spot: Изменения за 24ч
                if(bnSpot.size > 0) {
                    promises.push(fetch('https://api.binance.com/api/v3/ticker/24hr').then(r=>r.json()).then(data => {
                        if(!Array.isArray(data)) return;
                        data.forEach(t => { if(bnSpot.has(t.symbol)) { const tk=this.tickersMap.get(`${t.symbol}:binance:spot`); if(tk){ tk.change=parseFloat(t.priceChangePercent); tk.volume=parseFloat(t.quoteVolume); tk.trades=parseInt(t.count); }}});
                    }).catch(()=>{}));
                }
            }
            ...
            // Bybit Futures
            if (byFut.size > 0) {
                promises.push(fetch('https://api.bybit.com/v5/market/tickers?category=linear').then(r=>r.json()).then(data => {
                    if(data.retCode===0) data.result.list.forEach(t => { if(byFut.has(t.symbol)) { const tk=this.tickersMap.get(`${t.symbol}:bybit:futures`); if(tk){ tk.price=parseFloat(t.lastPrice); tk.change=parseFloat(t.price24hPcnt)*100; tk.volume=parseFloat(t.volume24h)*parseFloat(t.lastPrice); }}});
                }).catch(()=>{}));
            }
            // Bybit Spot
            if (bySpot.size > 0) {
                promises.push(fetch('https://api.bybit.com/v5/market/tickers?category=spot').then(r=>r.json()).then(data => {
                    if(data.retCode===0) data.result.list.forEach(t => { if(bySpot.has(t.symbol)) { const tk=this.tickersMap.get(`${t.symbol}:bybit:spot`); if(tk){ tk.price=parseFloat(t.lastPrice); tk.change=parseFloat(t.price24hPcnt)*100; tk.volume=parseFloat(t.volume24h)*parseFloat(t.lastPrice); }}});
                }).catch(()=>{}));
            }
    ...
    async addSymbolsBatch(symbolsData) {
        ...
        symbolsData.forEach(({ symbol, exchange, marketType }) => {
            ...
            const key = `${symbol}:${exchange}:${marketType}`; if (this.tickersMap.has(key)) return;
        ...
        this.filterCache = null; this.displayedTickers = []; this.tickerElements.clear();
        this.renderTickerList();
    }
    ...
    async fetchInitialDataForSymbol(symbol, exchange, marketType) {
        try {
            ...
            // ИСПРАВЛЕНИЕ: Принудительно пробиваем блокировку рендера
            this.updatePriceImmediate(); 
    ...
    handleStarClick(star) {
        ...
        handleContextMenu(e) {
        // ====================================================================
        // УНИВЕРСАЛЬНОЕ ИСПРАВЛЕНИЕ ТЕКСТОВОГО УЗЛА БРАУЗЕРА
        // ====================================================================
        let target = e.target;
        if (target && target.nodeType === 3) {
            target = target.parentElement;
        ...
        // 1. Клик по флагу — показываем меню флагов
        ...
        // ====================================================================
        // 2. ЖЕЛЕЗОБЕТОННАЯ ПРОВЕРКА СТРУКТУРЫ СЕТКИ
        // children[0] в вашей сетке это ВСЕГДА <div class="ticker-name">
        // Если клик был ВНЕ первой колонки — БЛОКИРУЕМ
        // ====================================================================
        const nameColumn = tickerItem.children[0];
        if (!nameColumn || !nameColumn.contains(target)) {
            return; 
        }
        
        // Если внутри колонки названия кликнули на звезду (★) или маркер (F/S) — тоже блокируем
        if (target.closest('.star') || target.closest('.market-sup')) {
            return; 
        }
        // ====================================================================
        ...
        if (this.watchlistManager && this.watchlistManager.lists) {
            ...
            this.watchlistManager.listOrder.forEach(listId => {
                ...
                if (list) {
                    html += `
                        <div class="context-menu-item" data-action="add-wl" data-list-id="${listId}" data-symbol="${symbol}" data-exchange="${exchange}" data-market-type="${marketType}">
    ...
    focusOnSymbol(symbol, exchange, marketType) {
        ...
        if (ticker && this.renderer) {
            ...
            if (index !== -1) {
                ...
                setTimeout(() => { this.renderer.renderVisibleTickers(); const el = document.querySelector(`.ticker-item[data-symbol="${symbol}"][data-exchange="${exchange}"][data-market-type="${marketType}"]`); if (el) el.classList.add('active'); }, 100);
            }
        }
    ...
    handleFlagSelect(e) {
        e.stopPropagation();
        const contextMenu = document.getElementById('flagContextMenu'), symbol = contextMenu.dataset.symbol, exchange = contextMenu.dataset.exchange, marketType = contextMenu.dataset.marketType, flag = e.currentTarget.dataset.flag;
        ...
        if (ticker) {
            ...
            if (tickerElement) { const flagContainer = tickerElement.querySelector('.flag, .flag-placeholder'); if (flagContainer) { const newFlag = document.createElement('div'); newFlag.className = `flag flag-${flag}`; newFlag.dataset.symbol = symbol; newFlag.dataset.exchange = exchange; newFlag.dataset.marketType = marketType; flagContainer.parentNode.replaceChild(newFlag, flagContainer); } }
        }
